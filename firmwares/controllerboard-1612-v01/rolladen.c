	...
	switch (frame->data[1])
	{
		...
		case HCAN_HES_ROLLADEN_POSITION_SET :
			if ( rolladen_is_in_group(p,frame->data[2]) ||
				 ( (p->config.taster == frame->data[2]) && //2-Taster-Betrieb
				   (frame->data[3] >= 200) ) ) //Taster-Rollladen-auf/zu-Nachricht
			{
				switch (p->lowlevel_state)
				{
					case ROLLADEN_STATE_STOP :
						if (frame->data[3] == 200) sollPos = 0;
						else if (frame->data[3] == 201) sollPos = 100;
						else sollPos = frame->data[3]; //in Prozent
					...
					{
							// muss gleich sein; hier nichts machen!
							p->soll_laufzeit = -1; // keine Soll-Laufzeit
					}
						break;
