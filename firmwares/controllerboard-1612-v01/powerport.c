...
{
	//x1.1: if (p->countDownTimer == 0) //nachladen verhindern
	{
		//wenn der Timer nicht schon/noch geladen ist

		// Timer initialisieren: Wenn das MSB == 1, dann
		// ist der Timerwert in Sekunden, ansonsten in Minuten
		// (p->config.gruppe3 = timer_delay)
...
void set_powerport_or_timer(device_data_powerport *p, uint8_t Flanke)
{
	if( Flanke == STEIGENDE_FLANKE )
	{
		...
		{
			relais_on(p); //sofort einschalten
	...
	else if( Flanke == FALLENDE_FLANKE )
	{
		...
		{
			relais_off(p); //sofort ausschalten
			p->countDownTimer = 0; //Timer aus
		}
	}
}

void relais_on(device_data_powerport *p)
{
...
void relais_off(device_data_powerport *p)
{
...
/*uint8_t get_relaisstate(device_data_powerport *p)
{
	return 0;
}*/
	...
	if(p->countDownTimer > 0) //Timer aktiv
	{
		if(p->countDownTimer == 1) //Timer abgelaufen
		{
			...
			if ( p->config.feature & (1<<POWERPORT_FEATURE_WECK_VERZOEGERT_EIN) )
			{
				...
				//x1.1: p->countDownTimer = 0; //fuer powerport_setup_timer
...
uint8_t powerport_is_in_group(const device_data_powerport *p, uint8_t group)
{
	...
	if (group == 255) //IL: neu in hcan2.0 von MH
		return 0;
	
	if (p->config.feature & (1<<POWERPORT_FEATURE_AUTO_AUS)) //Nachlauf aktiv
		maxPowerportGruppen--; //letzte Gruppe ist anders verwendet
...
void powerport_send_state_info(device_data_powerport *p)
{
	...
	message.data[3] = powerport_get(p) != 0; /* TODO: zusaetzlich Bit setzen,
	falls p->countDownTimer>0, damit in UI-PowerPortPage der Zustand blinkend
	dargestellt werden kann: Entspricht derzeit noch AUS bzw. noch EIN.
	Alternative den Counterwert anzeigen */
	message.size = 4;
